workspace "OpenStat - Vues Contexte et Conteneur" "Application de reporting pour les réseaux, services voix et vidéo, et services clients." {
    ...
    model {
        ...
        openstat = softwareSystem "OpenStat" "Application de reporting pour les réseaux, services de voix et de vidéo, et services clients et opérations." {
            ...
            refura = container "RefURA" "Gestion des authentifications et autorisations." "Java Spring Boot" {
                ...
                auth_service = component "Authentication Service" "Gère l'authentification des utilisateurs." "Spring Security"
                authz_service = component "Authorization Service" "Gère les autorisations et permissions des utilisateurs." "Spring Boot"
                user_mgmt_service = component "User Management Service" "Gère les informations des utilisateurs (création, mise à jour, suppression)." "Spring Boot"

                // === Interactions entre Composants ===
                auth_service -> user_mgmt_service "Valide les identifiants et récupère les informations utilisateur."
                authz_service -> auth_service "Demande l'état d'authentification."
                auth_service -> authz_service "Retourne l'état d'authentification pour autorisations."
                user_mgmt_service -> authz_service "Met à jour les permissions des utilisateurs."
            }

            openreport = container "OpenReport" "Calcul des KPI et analyses." "Java" {

                // === Composants Internes de OpenReport ===
                kpi_calculation = component "KPI Calculation Service" "Effectue les calculs des indicateurs clés de performance (KPIs)." "Java"
                data_mediation = component "Data Mediation Service" "Transforme et prépare les données collectées pour les calculs." "Java"
                analytics_engine = component "Analytics Engine" "Effectue les analyses avancées sur les données collectées." "Java"

                // === Interactions entre Composants ===
                kpi_calculation -> data_mediation "Demande les données transformées pour les calculs."
                data_mediation -> analytics_engine "Fournit les données pour les analyses avancées."
                analytics_engine -> kpi_calculation "Retourne les résultats d'analyse pour enrichir les calculs."
                //kpi_calculation -> openstat.refreport "Consulte les règles de calcul et les KPIs."
               // kpi_calculation -> Openstat.cassandra "Lit et écrit les données nécessaires aux calculs."
               // kpi_calculation -> openstat.nas "Stocke temporairement les résultats intermédiaires."
            }
    ...
    views {
        ...
        // Vue Déploiement pour RefURA et OpenReport
        deployment "Production" {
            include *
            autoLayout tb
            title "Vue de Déploiement - OpenStat"
            description "Cette vue détaille le déploiement des composants RefURA et OpenReport sur des nœuds de production."
        }
